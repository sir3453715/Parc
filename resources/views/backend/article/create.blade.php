...
@section('content')
    <div class="col-sm-10 blog-main">
        ...
        <form method="POST" action="/backend/article/{{Request::segment(3)}}/create" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="special">加入精選特輯:</label>
                <input name="special" type="checkbox" id="special">
            </div>
            {{-- Order --}}
            <div class="form-group">
                <label for="order">優先顯示於精選特輯:</label>
                <input name="order" type="checkbox" id="order">
            </div>
            @endif
            <div class="row">
                {{-- Category --}}               
            ...
            <div class="row" id="add_to_news" style="display:none;">
                {{-- Category2 --}}               
                <label for="Category">Category2:</label>
                <div class="col">
                    <div class="form-group">
                        <input type="text" value="" id="categorygg2" name="category2" hidden></option>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            // 2 category
                ...
                if(category_id2) {
                    $.ajax({
                        url: '/backend/article/create/ajax/'+category_id2,
                        type: "GET",
                        dataType: "json",
                        success:function(data) {
                            ...
                            $.each(data, function(key, value) {
                                $('select[name="sub_category2"]').append('<option value="'+ key +'">'+ value +'</option>');
                            });
            ...
            $('select[name="sub_category2"]').on('change', function() {
                var sub_category_id2 = $(this).val();
                var category_id2 = $('select[name="category2"]').val();
                if(sub_category_id2) {
                    $.ajax({
                        url: '/backend/article/create/ajax/'+category_id2+'/'+sub_category_id2,
                        type: "GET",
                        dataType: "json",
                        success:function(data) {
                            $('select[name="extra_sub_category2"]').empty();
        ...
        function toggle_add_to_news(){
            $("#add_to_news").toggle(500);
            $('select[name="sub_category2"]').val("");
            if($("#add_to_news_button").text() == "取消")
                $("#add_to_news_button").text("新增至最新消息");
            else
                $("#add_to_news_button").text("取消");
        }
    </script>
